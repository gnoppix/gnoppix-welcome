...
const RESPREFIX: &str = "/org/gnoppix/hello";
...
fn outdated_version_check(message: String) -> bool {
    ...
    let response = reqwest::blocking::get("https://gnoppix.org/versions.json");
...
fn connectivity_check(message: String) -> bool {
    let status = match reqwest::blocking::get("https://gnoppix.org") {
...
fn quick_message(message: String) {
    ...
    std::thread::spawn(move || {
        ...
        let cmd = "nusr/local/bin/calamares-online.sh".to_owned();
...
fn main() {
    ...
    // Set program name.
    glib::set_program_name("GnoppixHello".into());
    glib::set_application_name("GnoppixHello");
...
fn build_ui(application: &gtk::Application) {
    ...
    // Init window
    let builder: Builder = Builder::from_resource(&format!("{RESPREFIX}/ui/gnoppix-hello.glade"));
